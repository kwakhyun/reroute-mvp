...
오피스 이전이나 폐점 때 처분해야 하는 사무 자산을 필요한 확인 절차를 마친 인수처에 배분하고, 매각 대금, 절감한 폐기비와 운반비, 재사용률을 한 화면에서 비교하는 B2B 자산 매칭 MVP입니다.
...
현재 결정은 **소규모 실제 파일럿은 진행하되 전담 개발 인력 투입은 보류**하는 것입니다. 실제 기업 5곳에서 계약 주체와 지불 의사를 확인한 뒤, 유료 파일럿에 참여한 기업이 2곳 이상이고 배분안을 확정하겠다고 답한 사용자가 20% 이상일 때만 개발 인력 투입을 다시 검토합니다.
...
자산 담당자가 매각 대금, 폐기비와 운반비 절감액, 재사용률, 수거 횟수와 인수처 확인 자료를 한 화면에서 비교하면 확정 화면까지 더 쉽게 이동할 것이라고 가정했습니다.
...
- Playwright standalone 프로덕션 빌드 E2E 시나리오 4개 통과
...
공개 포트폴리오 배포에서는 샘플 데이터만 사용합니다. 현재 프로덕션은 공유 작업 중인 상태를 보호하기 위해 로그인 시 자동 초기화를 사용하지 않습니다. CI는 `main` 푸시마다 분석 재현성, 보안 감사, 타입/린트, 커버리지, 마이그레이션, 데모 리셋, 빌드와 Chromium E2E 시나리오를 실행합니다.

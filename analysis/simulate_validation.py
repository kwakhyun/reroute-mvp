...
def write_markdown(result: dict[str, Any]) -> None:
    ...
    report = f"""# 합성 파일럿 검증 시뮬레이션

> **중요:** 이 문서는 실제 고객 인터뷰나 운영 로그가 아닙니다. 실제와 유사한 의사결정 조건을 점검하기 위해 고정 시드로 생성한 합성 데이터이며, 사업성 검증 완료의 근거로 사용할 수 없습니다.

## Executive Summary

- **제한된 파일럿은 진행할 가치가 있습니다.** 기준 시나리오의 대표 합성 코호트는 입찰 근거 확인 {representative_counts['bids_opened']}/10명, 조건 탐색 {representative_counts['recalculation_opened']}/10명, 확정 의도 {representative_counts['confirmation_opened']}/10명으로 행동 기준을 통과했습니다.
- **전담 개발 투자는 보류해야 합니다.** 유료 파일럿 합성 중앙값은 {representative_counts['paid_pilot']}/5개사로 투자 기준인 2개사에 못 미쳤고, 확정 의도와 유료 파일럿을 함께 만족할 추정 확률도 {probability['investment_rule']:.1f}%에 그쳤습니다.
...
| 지표 | 결과 | 성공 기준 | 판정 |
| --- | ---: | ---: | --- |
| 입찰 근거 확인 | {representative_counts['bids_opened']}/10명 | 6명 이상 | 통과 |
| 조건 탐색 | {representative_counts['recalculation_opened']}/10명 | 3명 이상 | 통과 |
| 확정 의도 | {representative_counts['confirmation_opened']}/10명 | 2명 이상 | 통과 |
| 파일럿 의향 | {representative_counts['pilot_interest']}/5개사 | 3개사 이상 | 통과 |
| 유료 파일럿 | {representative_counts['paid_pilot']}/5개사 | 2개사 이상 | 미달 |

## 불확실성 결과

| 판단 기준 | 기준 시나리오 충족 확률 |
| --- | ---: |
| 입찰 근거 확인율 60% 이상 | {probability['bids_opened']:.1f}% |
| 조건 탐색률 30% 이상 | {probability['recalculation_opened']:.1f}% |
| 확정 의도율 20% 이상 | {probability['confirmation_opened']:.1f}% |
| 파일럿 의향 3/5개사 이상 | {probability['pilot_interest']:.1f}% |
| 유료 파일럿 2/5개사 이상 | {probability['paid_pilot']:.1f}% |
...
def main() -> None:
    ...
    result = {
        ...
        "decision": {
            ...
            "reason": "행동 기준은 방향성을 보이지만 유료 파일럿 기준과 전체 기준 동시 충족 확률이 낮음",
        },
        "required_disclaimer": "합성 시뮬레이션이며 실제 고객 검증 결과가 아님",
    }

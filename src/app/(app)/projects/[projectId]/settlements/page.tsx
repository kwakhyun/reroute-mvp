...
export default async function SettlementsPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const steps = [
    { title: "매칭안 확정", body: confirmed ? "수요처와 거래 금액이 확정되었습니다." : "승인자가 매칭안을 확정해야 합니다.", icon: CheckCircle, complete: confirmed },
  ...
  return (
    <div className="section-page">
      <ContentHeader backHref={`/projects/${projectId}/matching`} description="외부 결제사에서 확인한 입금과 지급 상태를 기록합니다. 이 화면에서 자금을 보관하거나 이체하지는 않습니다." eyebrow={dashboard.project.name} title="정산" />
      <div className="notice-banner settlement-disclosure"><WarningCircle aria-hidden="true" size={20} /><span><strong>외부 정산 안내</strong> 현재 MVP는 결제사에서 확인한 상태를 수동으로 기록하며, 자금을 보관하는 에스크로 기능은 제공하지 않습니다.</span></div>

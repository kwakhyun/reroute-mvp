  ...
  return (
    <form action={action} className="card project-create-form">
      ...
      <div className="form-grid">
        ...
        <label><span>배치 설명 <small>선택</small></span><input maxLength={120} name="batchLabel" placeholder="비우면 자산 수량으로 자동 생성" /></label>
      ...
      <div className="form-section-heading form-section-divider">
        <span>2</span>
        <div><h2>자산 CSV</h2><p>최대 500개 자산군, 1MB까지 한 번에 가져옵니다. 이미지는 템플릿에 포함된 4개 경로 중 하나를 사용합니다.</p></div>
      </div>
      <label className="file-drop-field">
        <FileCsv aria-hidden="true" size={30} />
        <strong>자산 CSV 파일 선택</strong>
        <span>정해진 7개 열을 포함한 UTF-8 CSV</span>

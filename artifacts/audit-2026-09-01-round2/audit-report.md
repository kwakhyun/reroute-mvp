...
기존 보고서의 “열린 P0, P1, P2 0건” 판정은 철회해야 한다. 당시 검증은 현재 시드 데이터의 정상 흐름, 화면 품질, 선택된 순수 함수 테스트에 집중했고, 운영 중 마이그레이션, 깨끗한 CI 환경, 동시 변경, 신규 프로젝트의 실제 완주성, 측정 지표의 중복 여부를 충분히 검증하지 않았다.
...
- GitHub Actions의 깨끗한 환경과 production standalone 기동 조건
- DB가 비어 있거나 마이그레이션되지 않은 상태의 readiness
- 신규 프로젝트 생성 후 실제 매칭까지 이어지는 운영 경로
- 화면에 표시된 최소 회수 목표와 추천 엔진의 실제 승인 규칙
- 퍼널 이벤트의 발생 위치와 중복
...
### 1. 프로젝트 목록 — 화면은 양호, 신규 프로젝트 완주성은 미흡

![프로젝트 목록](./02-projects.png)

프로젝트 상태, 자산 수량, 순경제효과와 진입 행동은 명확하다. 다만 신규 프로젝트는 자산 CSV까지만 받을 수 있고 입찰을 추가하는 화면, API, 운영자 도구가 없어 매칭 단계로 진행할 수 없다.

### 2. 추천안 검토 — 시각 구조는 양호, 승인 규칙은 모순

![추천 매칭안](./01-matching.png)

화면은 자산 최소 회수 목표 합계 `1,740만 원 이상`을 표시하지만, 의사결정 기준은 프로젝트 기준 `1,500만 원 이상`만 검사한다. 현재 추천안의 현금 회수액은 `1,560만 원`이라 자산 표의 목표보다 낮은데도 승인 가능으로 표시된다.

추천 표에는 수요처와 수량만 있고 자산군 열이 없다. 수량이 같은 자산군이 생기면 승인자가 어떤 자산이 어느 수요처로 배정되는지 본 화면에서 확인할 수 없다.
...
- 원인: 기존 `slot`을 `asset_group_id`로 옮길 때 A, B, C, D만 1~4번 자산군으로 매핑한다. 그 외 값은 `display_order = 0`과 조인되어 새 테이블에 복사되지 않지만, 바로 원본 `bids`를 삭제한다.
- 재현: 0001 이후 5번째 자산군과 `slot='E'` 입찰 1건을 넣고 0002를 적용했다. 결과는 `before=1`, `after=0`, `foreignKeyErrors=0`이었다.
- 추가 위험: 마이그레이션 끝에 `PRAGMA foreign_key_check`도 없어, 삭제된 입찰을 참조하는 기존 배정이 있으면 고아 데이터가 남을 수 있다.
- 조치: 슬롯 문자가 아닌 영속 자산 식별자로 백필하고, 복사 전후 행 수가 다르면 마이그레이션을 중단한다. 5개 이상 자산군, 비표준 슬롯, 기존 배정이 있는 업그레이드 테스트를 추가한다.
...
| P1-02 | 자산군별 최소 회수 목표가 승인 규칙에 사용되지 않는다. | [`matching-mutations.ts`](../../src/server/services/matching-mutations.ts)는 엔진에 자산군 `id`, `quantity`만 전달하고, [`matching-engine.ts`](../../src/server/services/matching-engine.ts)는 프로젝트 전체 현금 기준만 검사한다. 현재 화면은 1,740만 원 목표와 1,560만 원 승인안을 동시에 표시한다. | 자산군별 현금 하한을 실제 제약으로 강제하거나 필드와 화면을 `참고 추정액`으로 재정의한다. 두 개의 서로 다른 최소 기준을 동시에 승인 기준처럼 표시하지 않는다. |
| P1-03 | 신규 프로젝트는 실제 매칭으로 진행할 수 없는 막다른 흐름이다. | 저장소에서 `bids` 삽입은 [`scripts/seed.ts`](../../scripts/seed.ts)에만 있다. 생성 E2E도 자산 화면 확인에서 끝난다. | 최소한 내부 운영자용 입찰 CSV 가져오기, 검증 상태 설정, 오류 행 리포트와 재계산 진입을 제공한다. 외부 셀프서비스가 범위 밖이면 명시적인 운영자 인계 상태와 절차를 화면에 둔다. |
...
| P1-07 | 핵심 퍼널의 `bids_opened`가 정상 CTA 경로에서 두 번 기록된다. | [`matching-actions.tsx`](../../src/components/matching/matching-actions.tsx)의 링크 클릭과 [`bids/page.tsx`](../../src/app/(app)/projects/[projectId]/bids/page.tsx)의 페이지 진입이 같은 이벤트를 각각 전송한다. | 노출 또는 클릭 중 하나로 의미를 고정하거나 서로 다른 이벤트로 분리한다. 사용자, 프로젝트, 세션 기준 중복 제거 규칙과 퍼널 SQL을 테스트한다. |
| P1-08 | 승인 화면에서 자산군과 수요처의 배정 관계를 직접 확인할 수 없다. | [`match-proposal-table.tsx`](../../src/components/matching/match-proposal-table.tsx)는 자산군 이름을 받거나 표시하지 않는다. 수량이 같은 자산군에서는 추론도 불가능하다. | 조회 모델에서 자산군 ID와 이름을 반환하고 추천안, 확인 모달, 확정 스냅샷에서 배정 관계를 명시한다. |

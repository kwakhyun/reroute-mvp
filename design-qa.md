...
노란색 강조, 흰색 카드, 회색 작업 배경과 Noto Sans KR 제목을 전체 제품 화면에 적용했습니다. 홈의 서비스 소개 카드, 프로젝트 현황 요약, 작업 공간 헤더와 로그인 안내를 정리했습니다. [디자인 시스템](docs/design-system.md)에 현재 규칙을 기록했습니다.

홈, 로그인, 프로젝트, 자산, 매칭, 입찰, 수거, 정산과 새 프로젝트를 1440px 데스크톱과 390px 모바일에서 확인했습니다. 아래 2026-09-01 자료와 영상은 이전 UI의 검증 기록입니다. 현재 화면은 [홈](public/product/overview.png)과 [매칭](public/product/matching-preview.png)에서 확인할 수 있습니다.

- 프로덕션 빌드와 타입 검사, ESLint를 통과했습니다.
- Chromium E2E 12개 시나리오와 컴포넌트 테스트 3개를 통과했습니다. CSV 미리보기, 배분 확정, 동시 수정 거절과 수거 및 정산 기록 흐름을 포함합니다.
- 390px 로그인 입력창의 크기와 모바일 확정 버튼의 텍스트 영역을 회귀 검사에 추가했습니다. 모바일 확인창의 버튼을 세로로 배치해 긴 문구의 줄바꿈을 보완했습니다.
- 중간 화면 폭의 지표 겹침, 실제로 넘치는 표의 스크롤 안내, 메뉴의 포커스 복귀를 확인했습니다. 수동 화면 점검에서는 본문 가로 넘침과 브라우저 실행 오류가 없었습니다.

## 이전 UI 검증 기록

검증일: 2026-09-01

기준 상태: 성수 오피스 이전 프로젝트, 승인자 계정, 배분안 확정 전
...
19. 공개 화면, 제품 화면, CSV, README와 생성 보고서의 독자용 용어를 다시 점검했다. `유휴 자산`, `현금 회수액`, `매각 수익`, `총 경제 효과`, `가상 데이터 검증`처럼 뜻이 모호하거나 실제 데이터와 어긋나는 표현을 구체적인 일상어로 바꾸고, 1440×1024 전후 화면을 나란히 비교해 줄바꿈과 정렬을 확인했다.
...
- 문구와 콘텐츠: 수거 상태, 담당자, 결제사 확인과 가상 데이터 분석의 한계를 자연스러운 한국어로 유지했다.
